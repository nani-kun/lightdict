...
- **整页翻译：按钮、标签、导航项这类短文字的译文不再另起一行**，直接接在原文后面（「Home 首页」）。短标签本来就靠紧凑取胜，为两个字换一行会把工具栏、标签栏撑成两倍高，还常挤破固定高度的容器。判据是「一段不超过 30 个字符、且不是两句话」；标题（h1–h6）除外，字号大，另起一行更清楚。
- **译文跟着原文保留链接**：一段文字整个来自同一个 `<a>` 时，译文插进那个 `<a>` 里面，于是译文和原文一样能点，颜色、下划线、hover 也都是链接自己的。
- **修好一类漏翻：块级元素旁边掉队的按钮和链接**。`<div><button>关键词</button><div class="dropdown">…</div></div>` 这种写法（论文站的关键词条、各种下拉菜单的触发器都长这样）里，按钮的文字不属于任何一个块级子元素，原先只顺着块级往下走就把它整段漏掉了。现在会把这些掉队的行内元素单独收进来——但只在「块级子元素之外没有散落文字」时才收，免得把一句被 `<b>`、`<a>` 断成几截的话拆开翻成碎片。
- **一排并列的链接改成逐个翻译**：导航条、页脚链接、面包屑这种「块级元素里除了几个 `<a>`/`<button>` 再没有别的文字」的地方，原先整条一起送去翻，「Home Docs Blog Pricing」会被当成一句话译成不知所云的一串，译文还只能堆在整条导航的末尾；现在每项各翻各的，译文各回各的链接。

## [1.3.1] - 2026-08-28

- **默认引擎改用微软翻译**：Google 的接口在国内网络时常连不上，微软翻译走 `bing.com` 的免注册通道，直连稳定、长句也更通顺。划词翻译、中译英、整页翻译三处的默认值一起改；已经在设置里选过引擎的用户不受影响。
- **默认发音来源从「自动」改成「必应朗读」**：少一层探测，出声更快。想回到原来的行为，在设置页「发音 → 发音来源」选「自动」即可。
- 展示站 `site/index.html` 的演示卡片发音优先挑微软嗓音，跟商店里装完的实际听感对齐；README 与展示页上标注的默认引擎同步更新。
