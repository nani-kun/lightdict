...
- **整页翻译**（工具栏按钮或 `Alt+T`）→ 把英文网页就地变成中英对照：每段原文下面跟一段译文，字体、字号、颜色都沿用原文，再按一次恢复原样。
...
| `Alt+T` | 同上，不用打开弹窗（快捷键可在 `chrome://extensions/shortcuts` 改） |
| 点击工具栏图标 | 快速开关、生词本、导出 |
...
工具栏弹窗里的「整页翻译」按钮（或快捷键 `Alt+T`）会把当前英文网页变成中英对照，再按一次恢复原样。
