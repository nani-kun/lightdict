# LightDict 轻词典

[![CI](https://github.com/nani-kun/lightdict/actions/workflows/ci.yml/badge.svg)](https://github.com/nani-kun/lightdict/actions/workflows/ci.yml)
[![License: MIT](https://img.shields.io/badge/License-MIT-blue.svg)](LICENSE)
[![Manifest V3](https://img.shields.io/badge/Chrome-Manifest%20V3-4285F4.svg)](manifest.json)
[![零依赖](https://img.shields.io/badge/依赖-0-brightgreen.svg)](CONTRIBUTING.md#代码风格)

一个轻量的 Chrome 划词词典扩展（Manifest V3，零依赖、无需构建）。

- **选中单词** → 停留片刻，弹出卡片：中英音标、中文释义（按词性分组）、可展开的英英解释与例句、发音、加入生词本。
- **选中句子** → 停留片刻，弹出中文译文，并附原文对照。
- **中译英**（设置页开关，默认关闭）→ 打开后选中中文也响应：中文词给出拼音与英文对应词，中文句子译成英文；关闭时中文选区一律不弹卡片。
- **弹窗里查词翻译** → 不用先在页面上划词：点工具栏图标，在输入框里敲中文或英文，回车即可，结果和划词卡片一样（音标、释义、发音、加入生词本、复制）。手输的内容不受「中译英」开关约束。
- **整页翻译**（工具栏按钮，或快捷键 `Alt+T`／macOS 上 `⌥ Option + T`）→ 把外文网页就地变成双语对照：源语言自动识别（英、日、韩、俄、法、阿拉伯……都行），一律译成简体中文，每段原文下面跟一段译文，字体、字号、颜色都沿用原文，再按一次恢复原样。
- 卡片上的 🔈 读的始终是英文那一侧：英文词读词，中文词读它的英文对应词，句子读英文原文或英文译文。
- **先到先显示**：一张单词卡片的内容来自三路数据源（翻译、英汉词典、英英词典），哪一路先回来就先显示哪一路，还没到的部分留一条「加载中」的微光条——不会因为某一路慢，整张卡片就一起空等。
- 卡片渲染在 **Shadow DOM** 内，不受页面样式干扰，也不会污染页面；自动跟随系统深浅色。

<table>
  <tr>
    <td width="50%"><img src="docs/screenshots/word.png" alt="选中英文单词后弹出的词典卡片" /></td>
    <td width="50%"><img src="docs/screenshots/sentence.png" alt="选中句子后弹出的翻译卡片" /></td>
  </tr>
  <tr>
    <td align="center"><sub>选中单词：音标、中文释义、英英解释与例句</sub></td>
    <td align="center"><sub>选中句子：中文译文，下面附原文对照</sub></td>
  </tr>
  <tr>
    <td><img src="docs/screenshots/zh-word.png" alt="选中中文词后弹出的汉英卡片" /></td>
    <td><img src="docs/screenshots/dark.png" alt="深色主题下的词典卡片" /></td>
  </tr>
  <tr>
    <td align="center"><sub>中译英（默认关闭）：拼音 + 英文对应词</sub></td>
    <td align="center"><sub>自动跟随系统深浅色</sub></td>
  </tr>
</table>

**整页翻译**：每段原文下面跟一段译文，字体、字号、颜色、行高都沿用原文——标题还是标题，引用还是斜体。

<img src="docs/screenshots/page.png" alt="整页翻译：外文网页变成简体中文对照" width="900" />

<sub>以上截图由 `bash tools/make_shots.sh` 生成：headless Chrome 拍 `demo/` 下的演示页，跑的是真实的内容脚本，喂的是本地假数据。</sub>

## 安装

1. 打开 `chrome://extensions/`
2. 右上角打开 **开发者模式**
3. 点击 **加载已解压的扩展程序**，选择本目录
4. 任意网页刷新后即可划词（首次安装会自动打开设置页）

## 使用

| 操作 | 效果 |
| --- | --- |
| 选中一个英文单词 / 短语（≤4 词） | 词典卡片 |
| 选中一句话或一段话 | 中文翻译卡片 |
| 选中一个中文词（≤6 字，需开启中译英） | 汉英卡片：拼音 + 英文对应词 |
| 选中一句中文（需开启中译英） | 英文翻译卡片 |
| `Esc` / 点击卡片外部 | 关闭卡片 |
| 卡片上的 🔈 / ☆ / ⧉ | 发音 / 加入生词本 / 复制 |
| 点卡片上的音标（英 / 美）或拼音 | 分别试听英音 / 美音 / 中文原词 |
| 卡片底部的「来源」 | 本次结果实际来自哪些引擎 |
| 点击工具栏图标 → 「整页翻译」 | 整页变中英对照；按钮变成「恢复原文」，再点一次还原 |
| `Alt+T`（macOS 上是 `⌥ Option + T`） | 同上，不用打开弹窗（快捷键可在 `chrome://extensions/shortcuts` 改） |
| 点击工具栏图标 | 查词输入框、快速开关、生词本、导出 |
| 在弹窗输入框里输入中英文并回车 | 直接查词 / 翻译，结果里的 🔈 / ☆ / ⧉ 和卡片上一样 |
| 点弹窗结果里的音标（英 / 美）或拼音 | 分别试听英音 / 美音 / 中文原词 |

设置页（工具栏图标 → 打开设置）可调整：触发方式（直接划词 / 按住 Shift、Ctrl、Alt）、停留延迟、卡片主题、是否显示英文释义、自动发音、发音来源、语音合成的中英文嗓音（可试听）、翻译引擎、英汉词典、英英词典、自动降级、长度上限、网页翻译引擎与译文样式、中译英开关及其两个引擎、例外网站。设置存于 `chrome.storage.sync`，改动即时生效，无需刷新页面。

「中译英」默认关闭：关闭时选中中文什么都不会发生（选中英文照常查），打开后中文选区才会走汉英词典与中译英引擎。

单词卡片上英文释义默认展开，点标题行的「英文释义 ▴」可以就地折叠；不想每次都看到，就在设置页关掉「显示英文释义」。

单词卡片是**分批填满**的：翻译、英汉词典、英英词典三路同时发出，后台每拼出一块就推给卡片一次，
所以常常是中文释义先出现、音标随后补上、英文释义最后到位；还没到的那一块先占一条微光条。
某一路超过 6 秒还没回来，卡片就按已有内容定稿（微光条撤掉），它真到了再补进去；
一路引擎全挂或超过 12 秒仍无果就放弃那一路，其余部分照常显示。只有三路全军覆没才会显示「查询失败」。

## 目录结构

```
manifest.json              扩展清单（MV3）
src/
  background/service-worker.js   所有网络请求、结果缓存、生词本存取
  content/content.js             划词监听 + Shadow DOM 卡片（样式内联在文件顶部 CSS 常量）
  content/page-translate.js      整页翻译：找段落、批量送翻、把译文插回原文里
  common/settings.js             默认配置与读写封装
  common/voices.js               语音合成的嗓音挑选规则（内容脚本与设置页共用的普通脚本）
  common/engines.js              查词 / 翻译引擎注册表（新增引擎只改这里）
  options/                       设置页
  popup/                         工具栏弹窗（查词输入框 + 开关 + 生词本）
icons/                     图标（由 tools/make_icons.py 生成）
demo/preview.html          卡片预览页，用假数据跑真实 content.js，可直接用浏览器打开
demo/page.html             整页翻译的排版试验场，用假译文跑真实 page-translate.js
tools/check.mjs            不联网的静态自检：语法、manifest 引用、host_permissions 覆盖
tools/test-query.mjs       在 Node 里跑一遍后台查询逻辑（真实联网），用于排查数据源
tools/make_icons.py        重新生成图标
tools/make_shots.sh        重新生成 README 里的截图（headless Chrome 拍 demo/ 下的演示页）
tools/make_store_assets.sh 重新生成 Chrome 应用商店的截图与宣传图
tools/store/               商店素材的舞台页与版式模板
docs/screenshots/          README 用的截图
docs/store/                商店素材，外加 listing.md（上架要填的文案与权限理由）
```

## 网页翻译

工具栏弹窗里的「整页翻译」按钮（或快捷键 `Alt+T`，macOS 上按 `⌥ Option + T`）会把当前网页变成双语对照，再按一次恢复原样。
**源语言不用选，也不用配**：目标语言只有简体中文一个，所以扩展只需分清「这一段是不是已经是简体中文」，
剩下的交给翻译引擎自带的自动识别（Google 的 `sl=auto`、微软的 `auto-detect`、MyMemory 的 `Autodetect`）。
设置页和弹窗上显示的快捷键都是向浏览器问来的（`chrome.commands.getAll()`），
在 `chrome://extensions/shortcuts` 改过之后，两处显示的就是改后的那个；macOS 上 Chrome 直接给的是 `⌥T` 这样的符号写法。

**译文插在哪里**：不是在原文旁边另摆一份文档，而是把译文作为原文元素的**最后一个子节点**塞进去。
译文因此继承原文的字体、字号、颜色、行高与对齐方式——所谓「保持相同的样式」，靠的是继承而不是抄样式。
也正因为不是插一个兄弟节点，列表编号不会翻倍、表格不会多出一格、flex/grid 的子项数量不变。
只有一种情况会动到页面：段落自己是一行不换行的 flex 容器（导航条最典型）时，会临时给它加上
`flex-wrap: wrap`，让译文自己占一行；恢复原文时原样撤掉。

**哪些段落会被翻**：从 `<body>` 往下找「最内层的块级元素」——一段、一个标题、一个列表项、一个表格单元格。
行内元素（`span`、`a`、`b`……）不单独翻，跟着包住它的那一段一起走，译文才是完整的一句而不是碎片。
以下内容会跳过：`<code>` / `<pre>` 等代码类标签、输入框、`contenteditable`、标了 `translate="no"` 或
`notranslate` 的元素、没有渲染框的隐藏内容、没有成词文字的段落（编号、符号、单个字母），
以及本来就是简体中文的段落。

**怎么认出「不是简体中文」**：看的是字符属于哪套文字，不是具体语种——目标语言只有简体中文一个，
分得清这两者就够了。出现假名或谚文，就是日文、韩文，一定翻；出现成词的非汉字字母
（拉丁、西里尔、希腊、阿拉伯、天城、泰文……连着两个），且没被汉字淹没，也翻。
麻烦只在汉字上：中日韩和繁简共用这批字，所以**只剩汉字**的段落要看它就近声明的语言
（元素上的 `lang`，没有就用 `<html lang>`）——`ja` / `ko` / `zh-Hant` / `zh-TW` 里的汉字词照翻（繁体也会转成简体），
其余一律当简体中文放过。

**怎么送去翻译**：按可视顺序排队（先翻眼前看得见的，再往下，最后回头补视口上方的），
每批约 20 段 / 1200 字符，用换行拼成一次请求，最多三批同时在飞。
**同一批只放同一套文字的段落**：引擎的自动识别是「整个请求认一门语言」，
把俄文段和日文段拼进同一次请求，认出来的那一门会把另一门连蒙带猜地译错。
（假名、汉字、谚文算同一套，否则日文页里那些「全是汉字」的地名、机构名会把请求数翻好几倍。）后台按行拆回来对号入座；
行数对不上就换引擎，再不行把这一批对半拆开重试，实在译不出的段落保持原文不动。
连续三批失败就停手，页面右下角会给出提示。译文按「引擎 + 原文」缓存在
`chrome.storage.session` 里（一次浏览会话内有效，最多 3000 条），重复段落只翻一次。

页面后来新增的内容（无限滚动、SPA 换页）会被 `MutationObserver` 接住，隔 0.7 秒补翻一遍。

设置页「网页翻译」一节可以单独选引擎（不跟划词共用，因为整页的量大得多），
以及译文样式：与原文一致（默认）、略淡一点、虚线下划线。

触发靠的是 `activeTab` 权限——只在你点工具栏图标或按快捷键的那一刻对当前标签页生效，
装的时候不会多出任何权限提示。整页翻译只作用于主框架，`<iframe>` 里的内容不动。

## 引擎

共五类引擎，各自独立选择，全部是免登录的公开接口。英文方向三类：**翻译引擎**出整句译文，**英汉词典**出中文释义（顺带音标与发音），**英英词典**出英文释义与例句；查一个英文单词时三类并行请求，结果合并进同一张卡片。中文方向两类：**中译英引擎**把中文句子译成英文，**汉英词典**给中文词的英文对应词与拼音，两者同样并行。

**翻译引擎**

| 引擎 | 接口 | 特点 |
| --- | --- | --- |
| Google 翻译（默认） | `clients5.google.com`（失败切 `translate.googleapis.com`） | 综合最好，单词还会附带中文词性释义 |
| 微软翻译 | `www.bing.com/ttranslatev3` | 质量与 Google 相当，长句常更通顺（响应里的 `usedLLM` 表示该句由大模型翻译） |
| 有道翻译 | `aidemo.youdao.com/trans` | 中文译文更自然，国内网络可直连 |
| MyMemory | `api.mymemory.translated.net` | 开放翻译记忆库，匿名每日有免费额度 |
| SimplyTranslate | `simplytranslate.org/api/translate` | Google 的公共镜像，直连被拦时的备胎；上游限流较紧，建议配合自动降级 |

**英汉词典**

| 引擎 | 接口 | 特点 |
| --- | --- | --- |
| 有道词典（默认） | `dict.youdao.com/jsonapi` | 中文释义 + 英美音标 + 真人发音，覆盖最全 |
| 金山词霸 | `dict-mobile.iciba.com` | 按词性分组的短词条，一眼扫得完 |
| 有道联想 | `dict.youdao.com/suggest` | 接口最轻，响应最快，也收派生词 |

**英英词典**

| 引擎 | 接口 | 特点 |
| --- | --- | --- |
| 有道英英（默认） | `dict.youdao.com/jsonapi` | 普林斯顿 WordNet 的释义带例句，国内直连最快；WordNet 没收录的词改用同一次请求取回的柯林斯高阶双解（只取英文那半截），顺带给音标与真人发音 |
| 必应词典 | `cn.bing.com/dict/search` | 朗文式的整句释义，国内直连，词组也查得到；没有 JSON 接口，一次要下载整页 HTML（两三百 KB），是几家里最重的 |
| Free Dictionary | `api.dictionaryapi.dev` | 英文释义带例句，另有音标与真人发音；需能访问境外站点 |
| Wiktionary | `en.wiktionary.org/api/rest_v1` | 维基词典，冷僻词和短语也查得到；域名属 Wikimedia，中国大陆无法直连 |
| Datamuse | `api.datamuse.com/words` | 轻量英文释义，响应最快；需能访问境外站点 |

前两家在中国大陆可直连，所以排在前面、默认用第一家；后三家都在境外，能不能用取决于你的网络环境。

**中译英引擎**（同一批服务换个目标语言）

| 引擎 | 接口 | 特点 |
| --- | --- | --- |
| Google 翻译（默认） | `clients5.google.com` | 综合最好，单词还会附带英文对应词与拼音 |
| 微软翻译 | `www.bing.com/ttranslatev3` | 质量与 Google 相当，长句常更通顺 |
| 有道翻译 | `aidemo.youdao.com/trans` | 中文原文理解得更准，国内网络可直连 |
| MyMemory | `api.mymemory.translated.net` | 开放翻译记忆库，匿名每日有免费额度 |
| SimplyTranslate | `simplytranslate.org/api/translate` | Google 的公共镜像，直连被拦时的备胎 |

**汉英词典**

| 引擎 | 接口 | 特点 |
| --- | --- | --- |
| 有道汉英（默认） | `dict.youdao.com/jsonapi`（`ce` 词典） | 英文对应词按词性分组，另有拼音，覆盖最全 |
| 金山词霸 | `dict-mobile.iciba.com` | 词条短，常用义项覆盖得不错，不给拼音 |
| 有道联想 | `dict.youdao.com/suggest`（`le=en`） | 接口最轻，响应最快，也收词组 |

没有真人录音时会依次退到网络合成，最后才是浏览器自带的语音合成。三档如下：

| 档位 | 接口 | 说明 |
| --- | --- | --- |
| 必应朗读（首选） | `www.bing.com/tfettts` | Azure 的神经网络嗓音（美音 Ava、英音 Sonia、中文 Xiaoxiao），中英文都读得了，整句也行。和微软翻译共用同一份会话（IG + token + key），不必另外拿凭据。要 POST 才拿得到音频，内容脚本发不出去，由后台代取后以 `data:` 链接回传，并在内存里缓存最近 24 条 |
| 有道通用发音 | `dict.youdao.com/dictvoice` | 单词与短语是词库里的真人录音，最准；超过二十来个字符就回 500，所以只用于单词和短语 |
| 百度朗读 | `fanyi.baidu.com/gettts` | 纯 GET，不要 cookie 也不要令牌，链接直接交给 `<audio>` 就能播，中英文通吃；必应那条路走不通时顶上 |

网络合成只用于 400 字符以内的中英文：更长的文本必应要等十几秒才出声，交给本地朗读反而更快；中英以外的语言两家都不保证，也直接走本地。

设置页「发音」一节的**发音来源**可以改这条链：

| 选项 | 候选顺序 |
| --- | --- |
| 自动（默认） | 词典录音 → 必应 → 有道 / 百度 → 系统嗓音 |
| 必应朗读 | 词典录音 → 必应 → 系统嗓音 |
| 百度朗读 | 词典录音 → 百度 → 系统嗓音 |
| 只用系统嗓音 | 系统嗓音（一律不联网取发音） |

筛选规则在 `src/common/voices.js` 的 `ttsAllows()`，划词卡片、弹窗、设置页三边共用同一份。筛的是**卡片这一侧**而不是后台：卡片有七天缓存，改了设置得立刻生效，不能等缓存过期。

浏览器自带的语音合成是最后一档。嗓音由 `src/common/voices.js` 挑：先按语言与口音过滤，再从推荐名单里选（en-US 优先 Ava / Allison / Samantha / Alex，en-GB 优先 Daniel / Kate / Serena，中文优先 Ting-Ting），并避开 macOS 自带的一堆特效嗓音（Albert、Bad News、Bells、Boing、Bubbles、Zarvox……全都标着「本地」，不挡掉的话默认就会挑中排在最前的 Albert）。不满意可以在设置页「发音」一节直接指定嗓音，旁边有试听；这份规则被内容脚本与设置页共用，所以试听听到的就是卡片上会读出来的那个。

音标和发音取两本词典里先给出的那份（英汉优先）；两边都没有时，退回 Google 翻译给的音译。点音标时全程只走那个口音，不会因为放不出来就偷偷换成另一个。中文词卡片上标的是拼音，🔈 读的是排在最前的那个英文对应词，点拼音本身则念中文原词；整句卡片没有现成录音，短句照样走网络合成，长句才落到本地朗读。

设置页的「自动降级」默认开启：首选引擎失败时按注册表顺序依次尝试其余引擎；关掉则只用选中的那一个，失败即报错。

卡片底部会标出这次结果实际来自哪些引擎（例如 `来源 Google 翻译 + 有道词典`），降级发生时显示的是真正出结果的那个而不是你选的那个；命中本地缓存时会额外标注「本地缓存」。

整页翻译走的是同一批翻译引擎的批量入口 `lines(texts)`：把若干段用换行拼成一次请求再按行拆回来。
Google 的返回是把输入切成的若干截，每截都带着自己那段原文（`orig`）和它的译文（`trans`），
但切法随源语言而变：英文这类有空格分词的语言按句切，换行落在某一截的结尾；
中日韩则整批只回一截，换行原样留在 `orig` 和 `trans` 里。所以归位要两种形状都认——
数 `orig` 里的换行知道这一截跨了几段，再按 `trans` 里的换行拆开分派；两边对不上就抛错，
让上层换引擎或把这一批对半拆开重试，也好过把译文错位安到别的段落上。其余三家直接拆译文里的换行。
`MyMemory` 的 `langpair` 没有 `sl=auto` 那样的开关，源语言这一头写 `Autodetect` 由它自己认；
它单次请求还限 500 字符，整页翻译很容易超，建议只让它当降级备胎。

微软翻译比其它几家多一步握手：先打开一次 `bing.com/translator`，从页面里刮出 `IG` 与防滥用的
`token` / `key`，之后一小时内的请求复用这份会话，过期或被判 401 时自动重刮一次。
这条接口还要求带上同一次访问种下的 bing.com cookie，所以它是唯一一个用 `credentials: 'include'`
发请求的引擎（其余一律 `'omit'`）——也就是说，用它翻译时你的 bing.com cookie 会随请求一起发出。
介意的话换成别的引擎即可。

请求只在后台 service worker 发出，只携带选中的文本，不发送页面地址或身份信息。查询结果按「引擎组合 + 文本」在本地缓存 7 天（最多 600 条），可在设置页清空。

整页翻译要把页面正文送去翻译，这一步显然会把这些文字交给你选的翻译服务——它是你主动点按钮或按快捷键才发生的，
不点就一个字都不会外发；发出去的同样只有正文，不含页面地址、Cookie 或身份信息。

> 新增引擎只需在 `src/common/engines.js` 的 `TRANSLATE_ENGINES` / `CN_DICT_ENGINES` / `EN_DICT_ENGINES` / `ZH_TRANSLATE_ENGINES` / `ZH_DICT_ENGINES` 里加一项，设置页的下拉框会自动出现；别忘了把域名加进 `manifest.json` 的 `host_permissions`。

## 隐私

没有服务器，没有账号，没有埋点。离开你电脑的只有你主动要查的那段文字——它被直接发给你在设置页选定的第三方接口，
不含页面地址、页面标题、Cookie 或任何身份信息（唯一例外是微软翻译，Bing 的接口要求带 `bing.com` cookie）。
生词本与查询缓存只存在本机；设置存在 `chrome.storage.sync`，会随你的 Chrome 账号同步，这是浏览器的机制，扩展不参与。

完整说明见 [PRIVACY.md](PRIVACY.md)。

## 自测

```bash
node tools/check.mjs                      # 静态自检：语法 + manifest + 权限覆盖（不联网，CI 跑的就是它）
node tools/test-query.mjs                 # 跑默认样例
node tools/test-query.mjs ubiquitous "It works."   # 指定内容
node tools/test-query.mjs --engines       # 逐个体检所有引擎（看谁还活着）
node tools/test-query.mjs --engine=youdao --cn=iciba --en=wiktionary book   # 指定引擎组合
node tools/test-query.mjs --zh-engine=youdao --zh=iciba 人工智能    # 指定中译英引擎组合
node tools/test-query.mjs --no-zh 人工智能        # 模拟关掉中译英（应报「未开启中译英」）
node tools/test-query.mjs --no-fallback --cn=iciba book            # 关掉降级，只用指定的那个
node tools/test-query.mjs --page           # 走一遍整页翻译的批量通道（英文 + 日文两组），看分段对不对得上
node tools/test-query.mjs --page --page-engine=youdao "Sign in" "Read more"   # 指定引擎与段落
open demo/preview.html                    # 肉眼检查卡片样式与定位（?auto=word|text|zhword|zhtext 自动弹卡片）
open demo/page.html                       # 整页翻译的排版试验场（?auto=1 自动翻；?style=muted|dotted 换译文样式）
open 'demo/page.html?auto=1&lazy=1'       # 两秒后追加几段，检查新内容会不会被补翻
open 'demo/page.html?auto=roundtrip'      # 翻一遍再恢复，检查页面是不是一个字节不差地还原了
python3 tools/make_icons.py               # 重新生成图标（需要 Pillow）
bash tools/make_shots.sh                  # 重新生成 README 截图（需要 Chrome 与 Pillow）
bash tools/make_store_assets.sh           # 重新生成商店截图与宣传图（尺寸会自动校验）
```

**排查发音问题**：点卡片上的 🔈 / 音标 / 拼音时，内容脚本会把整条发音链路打进页面控制台
（录音候选、逐个播放成败、最终选中的嗓音、utterance 的 lang 与语速、朗读事件、队列状态，
外加一张 `console.table` 列出系统所有嗓音）。日志前缀是 `[LightDict 发音]`，走的是
`console.warn`——`console.log` 属于 Info 级，容易被控制台的级别过滤悄悄吞掉。内容脚本
加载时还会打一行 `[LightDict] 内容脚本已加载 vX.Y.Z`：看不到它就说明页面上跑的还是旧代码
（在 `chrome://extensions/` 重新加载扩展后，页面本身也要刷新一次）。

把 DevTools 控制台左上角的执行环境从 `top` 切到 `LightDict 轻词典`，还能手动试：

```js
__lightdict.voices()                // 列出系统所有嗓音
__lightdict.speak('hello', 'en')    // 用扩展的挑嗓音逻辑念（可加第三个参数 'uk' / 'us'）
__lightdict.speak('你好', 'zh')
__lightdict.try('Alex', 'hello')    // 指定嗓音名试念，用来横向比较
__lightdict.raw('hello', 'en-GB')   // 绕过扩展逻辑，直接交给系统念，用来对比
```

## 已知限制

- 这些公开端点都没有 SLA，高频使用可能被临时限流或下线；开启自动降级可以顶住单个引擎失效。
- MyMemory 免费额度按 IP 计算（约每天 5000 词），超出后返回错误提示。
- 微软翻译依赖 Bing 页面里的令牌，Bing 改版时可能失效；它也比其它引擎慢一点（首次要多一次握手），且需要能访问 `bing.com`。
- 只有有道词典、有道英英和 Free Dictionary 提供真人发音音频；英汉与英英都换成别的之后，🔈 会退化为浏览器自带的语音合成。
- 嗓音是各台电脑自己安装的，设置里存的是嗓音名：换一台机器若没有同名嗓音，会自动退回推荐名单，设置页里会把它标成「本机不可用」。
- 有道词典对词组（如 `give up`）给的是整句式解释而不是短词条，卡片会偏长；想要短词条可以换成金山词霸。
- 中译英是按「选区里出现汉字」判定方向的，中英混排的选区会整段译成英文。
- 汉英词典给的是对应词而不是英文释义，中文词卡片上没有英英解释；中文句子也没有真人录音，只能用浏览器语音合成朗读。
- 除中文外的其它语言（日文、法文……）仍按原来的方式处理：整段译成中文。
- 整页翻译只处理主框架，`<iframe>` 里的内容（评论区、嵌入的文档）不翻。
- 整页翻译的目标语言固定是简体中文，不可配置；已经是简体中文的段落原样跳过。
- 只剩汉字的段落靠 `lang` 属性判方向：没声明 `lang`、又通篇只有汉字的日文页面（少见）会被当成中文跳过。
- 同一批里若真混了两套文字，分批时会切开；但同一套文字里的不同语言（例如日文页上的一段韩文）仍在一批里，引擎只认一门，另一门可能译错。
- 译文是纯文本：原文里的链接、加粗、行内代码在译文那一行不会保留。
- 导航条这类由多个行内链接拼成的块会整块一起翻，得到的是一行并排的词，而不是逐项对齐的译文。
- 少数页面会因此变高：译文插在原文所在的盒子里，定死高度或用 `line-clamp` 截断的容器可能把译文挡住一部分。
- React / Vue 等框架重绘某一段时，会把插在里面的译文一并抹掉；滚动或页面再变动时会被重扫补上。
- 一段文字里夹着块级子元素时（`<div>正文<div>另一段</div></div>`），外层那截散装文字会被漏掉。

## 贡献

欢迎提 issue 和 PR。克隆下来就能直接加载调试，不需要 `npm install`。
新增引擎、代码风格、提交信息规范都写在 [CONTRIBUTING.md](CONTRIBUTING.md) 里。

版本变更见 [CHANGELOG.md](CHANGELOG.md)。

## 许可

[MIT](LICENSE) © 東雲实验室

本扩展调用的都是第三方的公开接口，本项目不隶属于 Google、微软、有道、金山词霸或其它任何一家；
各接口的可用性与条款由其提供方决定。
